...
if ($hassiteconfig) {
    ...
    if ($ADMIN->fulltree && optional_param('section', null, PARAM_TEXT) == 'elasticsettings') {
        $settings->add(new admin_setting_check(
            'search_elastic/connectiontest',
            get_string('connectiontest', 'search_elastic'),
            new server_ready_check()
        ));
    }

    $settings->add(new admin_setting_configtext(
        'search_elastic/hostname',
    ...
    $settings->add(new admin_setting_configtext(
        ...
        get_string('port_help', 'search_elastic'),
        9200,
        PARAM_INT
    ));
    ...
    $settings->add(new admin_setting_configtext(
        ...
        get_string('index_help', 'search_elastic'),
        'moodle',
        PARAM_ALPHANUMEXT
    ));

    $settings->add(new admin_setting_configtext(
        'search_elastic/sendsize',
        get_string('sendsize', 'search_elastic'),
        get_string('sendsize_help', 'search_elastic'),
        9000000,
        PARAM_ALPHANUMEXT
    ));

    $settings->add(new admin_setting_configtext(
        'search_elastic/apikey',
        get_string('apikey', 'search_elastic'),
        get_string('apikey_help', 'search_elastic'),
        ''
    ));

    $settings->add(new admin_setting_configtext(
        'search_elastic/connecttimeout',
        get_string('connecttimeout', 'search_elastic'),
        get_string('connecttimeout_help', 'search_elastic'),
        5,
        PARAM_INT
    ));

    $settings->add(new admin_setting_configtext(
        'search_elastic/timeout',
        get_string('timeout', 'search_elastic'),
        get_string('timeout_desc', 'search_elastic'),
        0,
        PARAM_INT
    ));
    ...
    $settings->add(new admin_setting_configcheckbox(
        'search_elastic/signing',
        get_string('signing', 'search_elastic'),
        get_string('signing_help', 'search_elastic'),
        0
    ));

    $settings->add(new admin_setting_configtext(
        'search_elastic/signingkeyid',
        get_string('signingkeyid', 'search_elastic'),
        get_string('signingkeyid_help', 'search_elastic'),
        '',
        PARAM_TEXT
    ));

    $settings->add(new admin_setting_configpasswordunmask(
        'search_elastic/signingsecretkey',
    ...
    $settings->add(new admin_setting_configtext(
        'search_elastic/region',
        get_string('region', 'search_elastic'),
        get_string('region_help', 'search_elastic'),
        'us-west-2',
        PARAM_TEXT
    ));
    ...
    $settings->add(new admin_setting_configcheckbox(
        'search_elastic/wildcardend',
        get_string('wildcardend', 'search_elastic'),
        get_string('wildcardend_help', 'search_elastic'),
        0
    ));

    $settings->add(new admin_setting_configcheckbox(
        'search_elastic/wildcardstart',
        get_string('wildcardstart', 'search_elastic'),
        get_string('wildcardstart_help', 'search_elastic'),
        0
    ));

    $settings->add(new admin_setting_heading('advsettings', get_string('advsettings', 'search_elastic'), ''));
    $settings->add(new admin_setting_configcheckbox(
        'search_elastic/logging',
        get_string('logging', 'search_elastic'),
        get_string('logging_help', 'search_elastic'),
        0
    ));
    ...
    $settings->add(new admin_setting_configcheckbox(
        'search_elastic/usesimplequery',
        get_string('usesimplequery', 'search_elastic'),
        get_string('usesimplequery_help', 'search_elastic', [
            'simple' => html_writer::link($simpleurl, $simpleurl),
        'complex' => html_writer::link(
            $complexurl,
            $complexurl
        )]),
        0
